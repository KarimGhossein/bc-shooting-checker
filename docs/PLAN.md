...
4. `map/render.js` — the biggest slice; being done in a few passes (report-click rendering, then Shooting Spots, then View Parcels/Reveal Road), not all at once.
   - Pass 1 (report-click rendering): ~~`OVERLAY_CATEGORY_KEYS`/`overlayCategoryLayers`, `renderForestServiceRoads()`, `renderMapOverlays()`, `cutblockColor()`/`cutblockStyle()`/`cutblockHighlightStyle()`.~~ **Done, v99** (`overlayLayer` — the `L.layerGroup().addTo(map)` these all draw into — deliberately left inline, confirming the v98 lesson below applied cleanly; every `*Popup()` builder these call also stayed inline, earmarked for a future `ui/report.js` instead).
   - Pass 2 (Shooting Spots): `renderActiveCutblockOutlines()`, `renderActiveCutblockBuffers()`, `renderNearbyRoads()`, `revealPotentialSpots()`/`revealPotentialSpotsAroundPin()`.
   - Pass 3 (View Parcels/Reveal Road): `drawParcelViewLayer()`, `revealAllParcels()`, `revealAllRoads()`, `parcelViewStyle()`, `parcelViewBboxCql()`.
   - **The v98 lesson, confirmed again in v99**: every `<script src>` file in this migration loads *before* the main inline script's `const map = L.map(...)` (line ~1603) runs, so any top-level statement that touches `map`, `L.layerGroup().addTo(map)`, `L.control(...)`, etc. *immediately* (not inside a function body) can't move into one of these files as-is — it'll throw on `map` not being defined yet. `overlayLayer` hit exactly this in v99, the same way `chooserHighlightLayer` did in v98. Passes 2 and 3 will hit it again — `spotLayer`/`roadViewLayer`/`parcelViewLayer` are all declared the same way. Each slice's own top comment should keep calling this out explicitly rather than silently leaving map-dependent globals inline without explanation.
